...
import { ReactComponent as RefreshIcon } from "remixicon/icons/System/refresh-fill.svg";
import { ReactComponent as ErrorWarningIcon } from "remixicon/icons/System/error-warning-line.svg";
...
const PdfDownloader = memo(({ plan, maxAttempts, onFlush }) => {
  ...
  return (
    <CardActivity type={status} loading={isPending}>
      <CardActivityIcon>
        {isPending && <RefreshIcon/>}
        {isResolved && <CheckboxCircleIcon/>}
        {isRejected && <ErrorWarningIcon/>}
      </CardActivityIcon>
